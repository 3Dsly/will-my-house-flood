...
export function initUI({onSearch,onLocate,onRiseChange}) {
  ...
  return {
    ...
    setBusy(busy){$('searchBtn').disabled=!!busy;$('locateBtn').disabled=!!busy;form.setAttribute('aria-busy',String(!!busy));},
    ...
    showResult(result){showDepth(result);$('readout').textContent=result.note;if(result.available)$('placeName').textContent=result.placeName;},
