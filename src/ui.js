import { updateRiseLabel } from './units.js';
...
export function initUI({onSearch,onRiseChange}) {
  ...
  slider.addEventListener('input',()=>{updateRiseLabel(Number(slider.value));onRiseChange(Number(slider.value));});
  ...
  return {
    ...
    setRise(rise){slider.value=String(rise);updateRiseLabel(rise);setExampleRise(rise);},
