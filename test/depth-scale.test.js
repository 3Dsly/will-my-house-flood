...
for (const height of [365,390,440]) {
  test(`6 m equals 6 / 1.7 human heights at ${height}px`,()=>{
    const g=depthGeometry(6,height);
    assert.ok(Math.abs(g.waterHeight/g.personHeight-6/1.7)<1e-10);
...
test('shallow water remains proportional rather than shrinking the human',()=>{
  for(const depth of [.1,.5,1,2,6,70,150]){
    ...
    assert.ok(Math.abs(g.waterHeight/g.personHeight-depth/1.7)<1e-10);
...
for (const height of [365,390,440]) {
  test(`30 m clearance equals 30 / 1.7 human heights at ${height}px`,()=>{
    const g=depthGeometry(-30,height);
    assert.ok(Math.abs(g.clearanceHeight/g.personHeight-30/1.7)<1e-10);
...
test('dry scale fits near-waterline and very high ground without changing person ratio',()=>{
  for(const d of [.1,1,2,30,100,1600]){
    ...
    assert.ok(Math.abs(g.clearanceHeight/g.personHeight-d/1.7)<1e-9);
